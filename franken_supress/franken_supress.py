...
from rubin_sim.scheduler.utils import (Footprint, Footprints, Sky_area_generator, Constant_footprint, Step_slopes,
                                       slice_wfd_area_quad, Sky_area_generator, ra_dec_hp_map, int_rounded)
...
def make_rolling_footprints(fp_hp=None, mjd_start=60218., sun_RA_start=3.27717639,
    ...
    if nslice == 2:
        rolling = [down, up, down, up, down, up, down, up]
    elif nslice == 3:
...
class filter_sched(simple_filter_sched):
    """
    Note, this might not work great if we need to restart a bunch. So a more robust 
...
def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
    ...
    n_visit_limit = None
    fs = filter_sched(illum_limit=illum_limit)
...
if __name__ == "__main__":
    ...
    if dbroot is None:
        ...
        fileroot = dbroot + '_'
    file_end = 'repeat%.1f_v2.99_' % np.abs(repeat_weight)
    ...
    # XXX--should combine twilight blobs and twilight NEO into a single 
    # survey object so that they don't have to be manually tolod about each other
    reverse_neo_night_pattern = [not val for val in neo_night_pattern]

    # Modify the footprint
    nes_scale = .25/.3  # scale down the NES a bit.
    ...
    footprints_hp_array, labels = sky.return_maps(
        ...
        nes_ratios={"g": 0.28*nes_scale, "r": 0.4*nes_scale, "i": 0.4*nes_scale, "z": 0.28*nes_scale},
        dusty_plane_ratios={"u": 0.08, "g": 0.12, "r": 0.31, "i": 0.31, "z": 0.28, "y": 0.22})

    # XXX--rename wfd_indx to rolling_indx
    ...
    greedy = gen_greedy_surveys(nside, nexp=nexp, footprints=footprints, repeat_weight=repeat_weight)
    ...
    blobs = generate_blobs(nside, nexp=nexp, footprints=footprints, mjd_start=conditions.mjd_start,
                           good_seeing_weight=gsw, repeat_weight=repeat_weight)
