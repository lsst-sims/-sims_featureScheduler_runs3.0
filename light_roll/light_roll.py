...
if __name__ == "__main__":
    ...
    parser.add_argument("--rolling_strength", type=float, default=0.5)
    ...
    scale = .25/.3  # scale down the NES a bit.
    sky = Sky_area_generator_galplane(nside=nside, smc_radius=4, lmc_radius=6)
    footprints_hp_array, labels = sky.return_maps(
        bulge_ratios = {"u": 0.2, "g": 0.6, "r": 1.2, "i": 1.1, "z": 0.82, "y": 0.6},
        nes_ratios={"g": 0.28*scale, "r": 0.4*scale, "i": 0.4*scale, "z": 0.28*scale},
        dusty_plane_ratios={"u": 0.08, "g": 0.14, "r": 0.31, "i": 0.31, "z": 0.28, "y": 0.20})
    ...
    footprints = make_flipped_rolling_footprints(fp_hp=footprints_hp, mjd_start=conditions.mjd_start,
                                                 sun_RA_start=conditions.sun_RA_start, nslice=nslice, scale=rolling_scale,
