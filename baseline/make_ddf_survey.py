...
def generate_ddf_scheduled_obs(mjd_start, data_file='ddf_grid.npz', flush_length=2, mjd_tol=15, expt=30.,
                               ...
                               nsnaps=[1, 2, 2, 2, 2, 2], sequence_limit=258, survey_length=10):
    ...
    # Crop the grid down to where it should be
    mjd_end = mjd_start + survey_length*365.25
    in_range = np.where((ddf_grid['mjd'] >= mjd_start) & (ddf_grid['mjd'] <= mjd_end))[0]
