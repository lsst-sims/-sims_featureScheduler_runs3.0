...
from rubin_sim.scheduler.utils import (SkyAreaGenerator, ConstantFootprint, 
...
from rubin_sim.utils import _hpid2_ra_dec, angular_separation, survey_start_mjd
import rubin_sim
...
def ddf_surveys(mjd_start, detailers=None, season_frac=0.2, euclid_detailers=None):
    obs_array = generate_ddf_scheduled_obs(mjd_start, season_frac=season_frac)
...
def generate_twilight_neo(nside, night_pattern=None, nexp=1, exptime=15,
    ...
    for filtername in filters:
        ...
        bfs.append(((SunAltHighLimitBasisFunction(alt_limit=sun_alt_limit)), 0))
...
def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
              extra_info=None, illum_limit=40., mjd_start=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_dither", type=float, default=0.7, help="Dither size for DDFs (deg)")
    ...
    # Record information about software versions to store in output.
    extra_info = {}
    exec_command = ''
    for arg in sys.argv:
        exec_command += ' ' + arg
    ...
    ddfs = ddf_surveys(mjd_start, detailers=details,
    ...
    run_sched(surveys, survey_length=survey_length, verbose=verbose,
              fileroot=os.path.join(out_dir, fileroot+file_end), extra_info=extra_info,
              nside=nside, illum_limit=illum_limit, mjd_start=mjd_start)
