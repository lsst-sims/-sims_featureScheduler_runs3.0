...
def ddf_surveys(detailers=None, season_frac=0.2, euclid_detailers=None, ss=29):
    obs_array = generate_ddf_scheduled_obs(season_frac=season_frac, ss=ss)
...
def run_sched(surveys, survey_length=365.25, nside=32, fileroot='baseline_', verbose=False,
              extra_info=None, illum_limit=40., filter_scheduler=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--uzy", dest='uzy', action='store_true')
    parser.add_argument("--ddf_per", type=float, default=6.)
    parser.set_defaults(uzy=False)
    ...
    if dbroot is None:
        ...
        fileroot = dbroot + '_'
    file_end = 'ddfper%0.1f' % ddf_per + label + '_v2.99_'
    ...
    # Set how many sequences in a "standard season (ss)"
    # when ss = 29, we got 6.62% of observations in DDFs.
    ss = 29/0.0662 * ddf_per*.01

    ddfs = ddf_surveys(detailers=details, season_frac=ddf_season_frac,
                       euclid_detailers=euclid_detailers, ss=ss)
    ...
    run_sched(surveys, survey_length=survey_length, verbose=verbose,
              ...
              nside=nside, illum_limit=illum_limit, filter_scheduler=fs)
