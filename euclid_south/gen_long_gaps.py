...
from rubin_sim.scheduler.surveys import (LongGapSurvey, ScriptedSurvey,
                                         BlobSurvey)
...
def blob_for_long(nside, nexp=2, exptime=30., filter1s=['g'],
    ...
    for filtername, filtername2 in zip(filter1s, filter2s):
        ...
        bfs.append((bf.AfterEveningTwiBasisFunction(time_after=time_after_twi), 0.))
        # XXX--move kwargs up
        bfs.append((bf.HaMaskBasisFunction(ha_min=HA_min, ha_max=HA_max), 0.))
        # don't execute every night
        bfs.append((bf.NightModuloBasisFunction(night_pattern), 0.))
        # possibly force things to delay
        bfs.append((bf.DelayStartBasisFunction(nights_delay=nights_delayed), 0.))
...
def gen_long_gaps_survey(footprints, nside=32, night_pattern=[True, True],
    ...
    for filtername1, filtername2 in zip(f1, f2):
        ...
        surveys.append(LongGapSurvey(blob[0], scripted,
